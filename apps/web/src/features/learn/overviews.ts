...
export const TACTIC_OVERVIEW: Record<string, string> = {
  ...
  swordfish:
    '[[x-wing|X-Wing]] one size up: a digit confined to the same three columns across three rows (or the same three rows across three columns). The three cross-lines are spoken for, so the digit leaves their other cells. The rows need not each have the digit in all three columns — two is enough.',
  jellyfish:
    'The fish pattern at size four — a digit restricted to four columns across four rows, or vice versa. Much harder to see than an [[x-wing|X-Wing]] despite being the same logic, because the four base lines rarely line up neatly.',
  'finned-x-wing':
    'An [[x-wing|X-Wing]] with one extra candidate — the fin — that spoils the clean rectangle. The eliminations an X-Wing would make still hold, but only for cells that also see the fin. Everywhere else the fin gives the digit an escape route.',
  'finned-swordfish':
    'A [[swordfish|Swordfish]] with one stray extra candidate (the fin). The Swordfish eliminations survive only where the target cell also sees the fin.',
  'finned-jellyfish':
    'A [[jellyfish|Jellyfish]] with a fin. As with the smaller finned fish, only eliminations that also see the fin are safe.',
  skyscraper:
    'Take a digit that forms two strong links (a row or column where it has exactly two candidates). If the two links share one end in a line, then the two far ends can’t both be false — one of them is the digit. Any cell that sees both far ends therefore can’t hold the digit. New to “strong link” and “weak link”? See [[strong-weak-links|Strong Links & Weak Links]].',
  '2-string-kite':
    'One strong link on a digit in a row, one in a column, meeting in a shared box. Whichever way each link resolves, the digit lands at one of the two loose ends. A cell that sees both loose ends can’t be the digit. See [[strong-weak-links|Strong Links & Weak Links]] for the vocabulary this lesson leans on.',
  'turbot-fish':
    'A short single-digit chain: strong link, weak link, strong link. The two ends of the chain can’t both be the wrong colour, so a cell seeing both ends loses the digit. [[skyscraper|Skyscraper]] and [[2-string-kite|2-String Kite]] are the two shapes this chain can take — see [[strong-weak-links|Strong Links & Weak Links]] if that pairing is new.',
  'xy-wing':
  ...
  'xyz-wing':
    'Like an [[xy-wing|XY-Wing]], but the pivot itself also holds Z (candidates X, Y, Z). Now Z is forced into the pivot or one of the two pincers, so a cell that sees all three of them loses Z.',
  'w-wing':
    'Two cells, not seeing each other, with the identical pair {X, Y}. If a [[strong-weak-links|strong link]] on X connects them — a unit where X has only two spots, one seeing each cell — then whenever neither cell is X, both are Y. Either way Y lands in one of the pair, so cells seeing both lose Y.',
  'xy-chain':
    'A chain of two-candidate cells strung together by alternating [[strong-weak-links|strong and weak links]] — each cell is a strong link between its own two candidates, and shares a digit with the cell after it. Follow it from either end: assuming one end isn’t the shared digit forces the other end to be it, and vice versa. One end is always the digit, so any cell seeing both ends can’t hold it.',
  'unique-rectangle':
